...
add_telegraf_http_output() {
  ...
  cat <<EOF | sudo tee -a /etc/telegraf/telegraf.conf.tmp > /dev/null
[[outputs.http]]
...
install_telegraf_deb() {
  ...
  if ls /tmp/telegraf/*.deb &>/dev/null; then
    ...
    dpkg -i /tmp/telegraf/*.deb || apt -f install -y
  else
    ...
    # curl -s https://repos.influxdata.com/influxdata-archive.key | gpg --dearmor -o /etc/apt/trusted.gpg.d/influxdata.gpg
    echo "deb [signed-by=/etc/apt/trusted.gpg.d/influxdata.gpg] https://repos.influxdata.com/ubuntu $(lsb_release -cs) stable" | tee /etc/apt/sources.list.d/influxdata.list
    apt update
