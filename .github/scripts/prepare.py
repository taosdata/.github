...
class TestPreparer:
    ...
    def _prepare_repositories_remote(self, host_config):
        ...
        if (self.inputs.get('specified_source_branch') == 'unavailable' and
            self.inputs.get('specified_target_branch') == 'unavailable' and
            self.inputs.get('specified_pr_number') == 'unavailable'):
            success, _ = self._execute_remote_command(host_config, f"cd {workdir}/TDinternal && git reset --hard && git clean -f && git remote prune origin && git fetch && git checkout {self.target_branch}")
            if not success:
                logger.info(f"Failed to prepare TDinternal on {host}")
                return False
        else:
            success, _ = self._execute_remote_command(host_config, f"cd {workdir}/TDinternal && git reset --hard && git clean -f && git remote prune origin && git fetch && git checkout {self.source_branch}")
            if not success:
                logger.info(f"Failed to prepare TDinternal on {host}")
                return False
        
        # Prepare community repository
        success, _ = self._execute_remote_command(host_config, f"cd {workdir}/TDinternal/community && git reset --hard && git clean -f && git remote prune origin && git fetch && git checkout {self.target_branch}")
        if not success:
    ...
    def _update_codes_remote(self, host_config):
        ...
        if self.enterprise:
            ...
            job_name = 'TDinternalCI'
            success = self._update_latest_merge_from_pr_remote(host_config, f"{workdir}/TDinternal", self.pr_number, job_name)
            if not success:
                return False
            success = self._update_latest_from_target_branch_remote(host_config, f"{workdir}/TDinternal/community")
            if not success:
                return False
        else:
            logger.info(f"Updating codes for community on {host}...")
            job_name = 'NewTest'
            success = self._update_latest_merge_from_pr_remote(host_config, f"{workdir}/TDinternal/community", self.pr_number, job_name)
            if not success:
                return False
            success = self._update_latest_from_target_branch_remote(host_config, f"{workdir}/TDinternal")
            if not success:
                return False
    ...
    def _update_latest_from_target_branch_remote(self, host_config, repo_path):
        ...
        # Pull latest code
        success, _ = self._execute_remote_command(host_config, f"cd {repo_path} && git remote prune origin && git pull")
        if not success:
            logger.error(f"Failed to pull latest code from target branch on {host}")
            return False
            
        # Log git history
        success, log = self._execute_remote_command(host_config, f"cd {repo_path} && git log -5")
        if success:
            log_content = f"{repo_log_name} log: {log}\n"
            success, _ = self._execute_remote_command(host_config, f"echo '{log_content}' >> {host_config['workdir']}/jenkins.log")
            if not success:
    ...
    def _update_latest_merge_from_pr_remote(self, host_config, repo_path, pr_number, job_name=''):
        ...
        # Pull latest code
        success, _ = self._execute_remote_command(host_config, f"cd {repo_path} && git pull")
        if not success:
        ...
        success, _ = self._execute_remote_command(host_config, f"cd {repo_path} && git fetch origin +refs/pull/{pr_number}/merge && git checkout -qf FETCH_HEAD")
        if not success:
            logger.error(f"Failed to fetch and checkout PR on {host}")
            return False
            
        # Log merged history
        success, log_merged = self._execute_remote_command(host_config, f"cd {repo_path} && git log -5")
        if success:
    ...
    def _update_submodules_remote(self, host_config):
        ...
        success, _ = self._execute_remote_command(host_config, f"cd {workdir}/TDinternal/community && git submodule update --init --recursive")
        if not success:
    ...
    def _process_remote_hosts(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
            ...
            future_to_host = {
                ...
                for host_config in self.host_configs if host_config['host'] != self.local_ip
            }
