...
# Allow LAN traffic (192.168.0.0/22 covers 192.168.0-3.0/24, and 192.168.100.0/24 is added separately)
# 允许局域网通信（192.168.0.0/22 包含 192.168.0-3.0/24，单独添加 192.168.100.0/24）
# Note: iptables -d does NOT support comma-separated addresses; use separate rules instead.
# 注意：iptables -d 不支持逗号分隔多地址，需分开写两条规则
sudo iptables -A INPUT -s 192.168.0.0/22 -j ACCEPT
sudo iptables -A INPUT -s 192.168.100.0/24 -j ACCEPT
sudo iptables -A OUTPUT -d 192.168.0.0/22 -j ACCEPT
