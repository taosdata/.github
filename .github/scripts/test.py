...
class TestRunner:
    ...
    def run_function_test(self):
        ...
        linux_cmds = [
            ...
            f"date",
            f"cd {self.wkc}/test/ci && timeout 86400 time ./cov/run_coverage_func.sh -e -m /home/m.json -t cases.task -b {branch_id} -l {self.wkdir}/log -o 1230 {self.utils.get_env_var('extra_param')}".strip(),
        ]
        mac_cmds = [
            "date",
